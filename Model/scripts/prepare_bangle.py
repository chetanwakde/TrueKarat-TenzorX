...
def main():
    ...
    repo_path = os.path.join(raw_dir, "BangleFIR2")
    if not os.path.exists(repo_path):
        print(f"Warning: {repo_path} not found.")
        return
        
    images = find_images(repo_path)
    print(f"Found {len(images)} images for BangleFIR (Type 1)")
    ...
    for idx, img_path in enumerate(images):
        new_filename = f"BangleFIR_{idx}.jpg"
    ...
    print(f"Successfully appended {len(df_new)} bangle images! Total images now: {len(df_combined)}")
